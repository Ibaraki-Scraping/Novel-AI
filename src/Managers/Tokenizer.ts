...
function char(charCode: number): string {
    return String.fromCodePoint(charCode)
}

const buildByteEncoderDecoder = (): { byteDecoder: DataDecoder; byteEncoder: DataEncoder } => {
...
export class Encoder {
    ...
    private tokenizersNames = {
        "6B-v4": "gpt2",
        "euterpe-v2": "gpt2",
        "krake-v2": "pile",
        "genji-python-6b": "gpt2",
        "genji-jp-6b-v2": "gpt2-genji",
        "hypebot": "gpt2",
        "infillmodel": "gpt2",
    ...
    private tokenizersFolder = __dirname + "/../../Tokenizers/";

    private tokenizers = {
        "gpt2": this.tokenizersFolder + "gpt2_tokenizer.json",
        "gpt2-genji": this.tokenizersFolder + "genji_tokenizer.json",
        "pile": this.tokenizersFolder + "pile_tokenizer.json",
        "clip": null,
        "nerdstash_v1": this.tokenizersFolder + "nerdstash_tokenizer.model",
        "nerdstash_v2": this.tokenizersFolder + "nerdstash_tokenizer_v2.model",
    }

    constructor(model: keyof typeof Encoder.prototype.tokenizersNames) {
        const tokenizer = JSON.parse(readFileSync(this.tokenizers[this.tokenizersNames[model]]).toString());
        
        this.vocab = tokenizer.vocab
        this.merges = tokenizer.merges
        this.specials = tokenizer.specialTokens
            .map((special) => [special, tokenizer.vocab[special]])
            .reduce((obj: Vocabulary, [key, value]) => {
                obj[key] = value as number
                return obj
            }, {})
        this.config = tokenizer.config
    ...
    private splitWords(text: string): string[] {
        ...
        const splitRegex = this.splitRegex
        let accumulated = ''
    ...
    private toBPE(text: string): number[] {
        ...
        for (;;) {
            const bigram = rankedPairs[0].bigram
